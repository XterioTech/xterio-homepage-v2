...
const LatestNews = async () => {
  ...
  const socialIcons = <SocialIcons className="news-header__social" />

  return (
    <div className="latest-news" data-slice-backgroundcolour={'dark'}>
      <div className="latest-news__container">
        <LatestNewsHeader title={title as string} socialIcons={socialIcons} />
        ...
        <div className="latest-news__blocks">
          {news_item.map(({ image, category, title, author, link }, index) => (
            <LinkBase
              href={link}
              key={index}
              className="latest-news__block news-block"
            >
              <Image
                image={image}
